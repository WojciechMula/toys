...
void verify() {
	for (int i=0; i < 256; i++) {
        ...
        const uint64_t swar3 = convert_to_bin::swar3(i);
        const uint64_t simd  = convert_to_bin::simd(i);
        const uint64_t pdep  = convert_to_bin::pdep(i);

        if (naive != swar) {
            std::printf("failed SWAR for %d: %016lx != %016lx\n", i, naive, swar);
            return;
        }

        if (naive != swar2) {
            std::printf("failed SWAR2 for %d: %016lx != %016lx\n", i, naive, swar2);
            return;
        }

        if (naive != swar3) {
            std::printf("failed SWAR3 for %d: %016lx != %016lx\n", i, naive, swar3);
            return;
        }

        if (naive != simd) {
            std::printf("failed SIMD for %d: %016lx != %016lx\n", i, naive, simd);
            return;
        }

        if (naive != pdep) {
            std::printf("failed PDEP for %d: %016lx != %016lx\n", i, naive, pdep);
            return;
        }
	}
