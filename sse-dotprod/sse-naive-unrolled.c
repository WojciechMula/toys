float sse_naive_unrolled(const float* a, const float* b, size_t n) {
    ...
    for (size_t i=0; i < n; i += 4*4) {
        ...
        dots0 = _mm_add_ps(dots0, AB0);
        dots1 = _mm_add_ps(dots1, AB1);
        dots2 = _mm_add_ps(dots2, AB2);
        dots3 = _mm_add_ps(dots3, AB3);
    }

    dots0 = _mm_add_ps(dots0, dots1);
    dots2 = _mm_add_ps(dots2, dots3);
    dots0 = _mm_add_ps(dots0, dots2);

    float result = dots0[0] + dots0[1] + dots0[2] + dots0[3];
