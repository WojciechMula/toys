...
func (l *Lookup) write(ctx *generateContext) {
	...
	ctx.writeln("static constexpr const char lookup[%d][%d] = {", len(l.indices), len(l.words[0].word))
	ctx.indent += 4
	...
	ctx.writeln("static constexpr const %s value[%d] = {", ctx.valtype, len(l.indices))
	ctx.indent += 4
	for _, idx := range l.indices {
		if idx == -1 {
